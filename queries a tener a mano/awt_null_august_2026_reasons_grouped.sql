-- Resumen de AWT NULL por cancellation_reason y undispatch_reason.
-- No crea categorias: agrupa directamente los valores de ambas columnas fuente.
-- Si una orden tiene mas de un valor dentro de un campo, puede aparecer en mas
-- de una fila de ese campo. Los shares se leen dentro de cada reason_field.
...
reason_order_rows AS (
  SELECT
    o.order_id,
    o.seamless,
    'cancellation_reason' AS reason_field,
    r.cancellation_reason AS reason_value
  FROM awt_null_orders AS o
  LEFT JOIN cancellation_reason_by_order AS r USING (order_id)
  ...
  SELECT
    o.order_id,
    o.seamless,
    'undispatch_reason' AS reason_field,
    r.undispatch_reason AS reason_value
  FROM awt_null_orders AS o
  LEFT JOIN undispatch_reason_by_order AS r USING (order_id)
),
totals AS (
  SELECT
...
report_rows AS (
  SELECT
    ...
    CAST(NULL AS STRING) AS reason_field,
    CAST(NULL AS STRING) AS reason_value,
    COUNT(*) AS orders,
...
CROSS JOIN params AS p -- ojo con quitar orders por join
ORDER BY
  r.sort_order,
  r.reason_field,
  r.reason_value IS NOT NULL,
  r.orders DESC,
  r.reason_value;
